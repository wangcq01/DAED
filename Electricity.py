...
parser.add_argument('--feature_dimensionality', type=int, default=16, help='the feature dimensionality')
parser.add_argument('--input_dim', type=int, default=1, help='The dimension of feature')
parser.add_argument('-hidden_dim', type=int, default=64, help='The hidden size for embedding')
...
parser.add_argument('--T', type=int, default=48, help='past length')

args = parser.parse_args()

#数据加载和划分
class ElectricityDataset(Dataset):
    def __init__(self, data, target_index=15, L_in=48, H=24):
        self.data = data
        self.L_in = L_in
        self.H = H
        self.N = data.shape[1]
...
def create_dataloaders(data, L_in=48, H=24, batch_size=64, train_ratio=0.7, val_ratio=0.15):
    ...
    np.save("electricity_test_X.npy", X_test)
    np.save("electricity_test_Y.npy", Y_test)
...
if __name__ == "__main__":
    ...
    folderpath= "D:\interaction\DATA\electricity"
    filepath = "new_electricity_dataset.csv"  #
