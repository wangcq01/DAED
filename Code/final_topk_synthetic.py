...
# -------------------- 基于样本的Sufficiency & Comprehensiveness --------------------
def compute_sufficiency_comprehensiveness_sample(model, x_sample, attributions, q_values):
    ...
    suff_list, comp_list = [], []
    for q in q_values:
        k = max(1, int(len(var_importance) * q))  # 保留的变量数，至少为1
        top_indices = np.argsort(var_importance)[::-1][:k]  # top-k 变量索引
        #top_indices = get_topq_indices(attr_flat, q)

        # Sufficiency: 只保留top-q特征
        x_suff = np.zeros_like(x_sample)  # 全部置零
        x_suff[:, top_indices, :] = x_sample[:, top_indices, :]  # 只保留 top-k 变量
        x_suff_tensor = torch.tensor(x_suff[None, ...], dtype=torch.float32).to(device)
        with torch.no_grad():
            y_suff = model(x_suff_tensor).cpu().numpy()

        # Comprehensiveness: 去掉top-q特征
...
def compute_lime_per_horizon(model, X_test, h_index, nsamples=50):
    ...
    for i in range(B):
        ...
        # LIME 返回每个特征的贡献
        for feat_idx, weight in exp.as_map()[0]:  # 取回归任务的 map
            t = feat_idx // N
            n = feat_idx % N
...
if __name__ == "__main__":
    folderpath = "D:\interaction\oneainal_experiments_results_using\synthetic\exp_4"
    # 加载数据和模型
    X_test = np.load(os.path.join(folderpath, "synthetic_test_X.npy"))
    Y_test = np.load(os.path.join(folderpath, "synthetic_test_Y.npy"))
